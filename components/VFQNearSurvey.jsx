...
export default function VFQNearSurvey() {
  ...
  // 페이지뷰 / 단계 트래킹
  useEffect(() => {
    if (stage === "landing") track("vfq_landing_view");
  ...
  const handleShare = async () => {
    ...
    const url =
      ...
      "https://afternoon.clop.ai/vfq-near";
    const text = "내 근거리 시력, 1분 만에 점검해 보세요. (애프터눈)";
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        await navigator.share({ title: "AfterNOON · 근거리 시력 자가점검", text, url });
        track("vfq_share_click", { method: "web_share_api" });
      } catch (_) {
        /* user cancelled */
      }
    } else {
  ...
  const copyLink = async () => {
    ...
    try {
      ...
      track("vfq_share_click", { method: "copy_link" });
    } catch {
    ...
    <div style={{ background: C.bg, minHeight: "100vh", color: C.text }}>
      <div
        style={{
          ...
          margin: "0 auto",
          minHeight: "100vh",
          background: stage === "landing" ? C.card : C.bg,
          position: "relative",
          paddingBottom: 24,
        }}
      >
